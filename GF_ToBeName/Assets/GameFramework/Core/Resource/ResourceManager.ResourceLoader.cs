...
namespace GameFramework.Resource
{
    internal sealed partial class ResourceManager : GameFrameworkModule, IResourceManager
    {
        ...
        private sealed partial class ResourceLoader
        {
            ...
            private bool LoadDependencyExistAsset(string assetName, int priority, LoadResourceTaskBase mainTask,
                object userData)
            {
                if (mainTask == null)
                {
                ...
                {
                    if (!LoadDependencyExistAsset(dependencyAssetName, priority, dependencyTask, userData))
                    {
                        return false;
                    }
                }
                ...
                {
                    dependencyTask.LoadMain(null, resourceObject);
                    dependencyTask.OnLoadDependencyAssetSuccess(resourceObject.Target);
                    foreach (var temResourceObject in dependencyTask.GetDependencyResources())
                    {
                        dependencyTask.ResourceObject.AddDependencyResource(temResourceObject);
                    }
                }

                ReferencePool.Release(dependencyTask);
                return true;
            }
                ...
                object userData)
            {
                ...
                if (m_AssetPool.CanSpawn(assetName))
                {
                    AssetObject assetObject = m_AssetPool.Spawn(assetName);
                    foreach (string dependencyAssetName in dependencyAssetNames)
                    {
                        if (!LoadDependencyExistAsset(dependencyAssetName, priority, mainTask, userData))
                        {
                    ...
                    mainTask.OnLoadAssetSuccess(null, assetObject.Target, 0f);
                    ReferencePool.Release(mainTask);
                    return;
                }
            ...
            /// <param name="loadAssetCallbacks">加载资源回调函数集。</param>
            /// <param name="userData">用户自定义数据。</param>
            public void LoadAssetSync(string assetName, Type assetType,
                LoadAssetCallbacks loadAssetCallbacks, object userData)
            {
                ...
                LoadAssetTask mainTask = LoadAssetTask.Create(assetName, assetType, 0, resourceInfo,
                    dependencyAssetNames, loadAssetCallbacks, userData);
                ...
                if (m_AssetPool.CanSpawn(assetName))
                {
                    ...
                    foreach (string dependencyAssetName in dependencyAssetNames)
                    {
                        if (!LoadDependencyExistAsset(dependencyAssetName, 0, mainTask, userData))
                        {
                            string errorMessage = Utility.Text.Format(
                                "Can not load dependency asset '{0}' when load asset '{1}'.", dependencyAssetName,
                                assetName);
                    ...
                    asset = (UnityEngine.Object)assetObject.Target;
                }
                else
                {
                    foreach (string dependencyAssetName in dependencyAssetNames)
                    {
                        if (!LoadDependencyAssetSync(dependencyAssetName, 0, mainTask, userData))
                        {
                            string errorMessage = Utility.Text.Format(
                                "Can not load dependency asset '{0}' when load asset '{1}'.", dependencyAssetName,
                                assetName);
                    ...
                    if (!m_ResourcePool.CanSpawn(resourceInfo.ResourceName.Name))
                    {
                        string fullPath = Utility.Path.GetRegularPath(Path.Combine(
                        ...
                        UnityEngine.AssetBundle assetBundle = UnityEngine.AssetBundle.LoadFromFile(fullPath);
                        ResourceObject temResourceObject = ResourceObject.Create(resourceInfo.ResourceName.Name, assetBundle,
                            m_ResourceManager.m_ResourceHelper, m_ResourceManager.m_ResourceLoader);
                        m_ResourceManager.m_ResourceLoader.m_ResourcePool.Register(temResourceObject, true);
                    }
                    ...
                    if (assetType != null)
                    {
                        asset = ((UnityEngine.AssetBundle)(resourceObject.Target)).LoadAsset(assetName, assetType);
                    }
                    else
                    {
                        asset = ((UnityEngine.AssetBundle)(resourceObject.Target)).LoadAsset(assetName);
                    }
                    
                    AssetObject assetObject = AssetObject.Create(assetName, asset, resourceObject.Target, m_ResourceManager.m_ResourceHelper, m_ResourceManager.m_ResourceLoader);
                    m_AssetPool.Register(assetObject, true);
                    m_AssetToResourceMap.Add(asset, resourceObject.Target);
                }
                
                mainTask.OnLoadAssetSuccess(null, asset, 0f);
                ReferencePool.Release(mainTask);
            }

            private bool LoadDependencyAssetSync(string assetName, int priority, LoadResourceTaskBase mainTask,
                object userData)
            {
                if (mainTask == null)
                {
                ...
                foreach (string dependencyAssetName in dependencyAssetNames)
                {
                    if (!LoadDependencyAssetSync(dependencyAssetName, priority, dependencyTask, userData))
                    {
                        return false;
                    }
                }
