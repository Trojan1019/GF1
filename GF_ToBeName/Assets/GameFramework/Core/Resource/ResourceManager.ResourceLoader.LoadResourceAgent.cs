        ...
        private sealed partial class ResourceLoader
        {
            ...
            private sealed partial class LoadResourceAgent : ITaskAgent<LoadResourceTaskBase>
            {
                ...
                public StartTaskStatus Start(LoadResourceTaskBase task)
                {
                    ...
                    if (!m_Task.IsScene && !m_Task.IsDependencyAsset)
                    {
                    ...
                    if (m_Task.TotalDependencyAssetCount > 0)
                    {
                        return StartTaskStatus.HasToWait;
                    }
                ...
                private void OnResourceObjectReady(ResourceObject resourceObject)
                {
                    ...
                    if (m_Task.IsDependencyAsset)
                    {
                        s_LoadingAssetNames.Remove(m_Task.AssetName);
                        m_Task.Done = true;

                        m_Task.OnLoadDependencyAssetSuccess(resourceObject.Target);

                        foreach (var temResourceObject in m_Task.GetDependencyResources())
                        {
                            m_Task.ResourceObject.AddDependencyResource(temResourceObject);
                        }
                    }
                }
                ...
                {
                    m_ResourceLoader.m_ResourceManager._onBundleLoadSuccessCallBack?.Invoke(e.Resource);
                ...
                private void OnLoadResourceAgentHelperLoadComplete(object sender, LoadResourceAgentHelperLoadCompleteEventArgs e)
                {
                    ...
                    if (assetObject == null)
                    {
                        assetObject = AssetObject.Create(m_Task.AssetName, e.Asset, m_Task.ResourceObject.Target,
                            m_ResourceHelper, m_ResourceLoader);
                        m_ResourceLoader.m_AssetPool.Register(assetObject, true);
                        m_ResourceLoader.m_AssetToResourceMap[e.Asset] = m_Task.ResourceObject.Target;
