...
namespace GameFramework.ObjectPool
{
    internal sealed partial class ObjectPoolManager : GameFrameworkModule, IObjectPoolManager
    {
        ...
        private sealed class ObjectPool<T> : ObjectPoolBase, IObjectPool<T> where T : ObjectBase
        {
            ...
            /// <param name="name">对象名称。</param>
            /// <returns>要检查的对象是否存在。</returns>
            public void GetAssetAndUnSpawn(string name)
            {
                if (name == null)
                {
                    throw new GameFrameworkException("Name is invalid.");
                ...
                if (m_Objects.TryGetValue(name, out objectRange))
                {
                    foreach (Object<T> internalObject in objectRange)
                    {
                        ...
                        {
                            throw new GameFrameworkException(Utility.Text.Format("Can not find target in object pool '{0}', target name is '{1}'", new TypeNamePair(typeof(T), Name), name));
                        }
                    }
                }
            }
