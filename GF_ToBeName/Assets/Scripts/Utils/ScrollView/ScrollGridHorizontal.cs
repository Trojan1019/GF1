...
public class ScrollGridHorizontal : MonoBehaviour
{
    ...
    public void Init() { 
        ...
        this.scrollRect.vertical = enableVertical;
        this.scrollRect.horizontal = enableHorizontal;
        this.scrollRect.movementType = ScrollRect.MovementType.Clamped;
        GameObject viewport = new GameObject("viewport", typeof(RectTransform));
        viewport.transform.parent = transform;
        viewport.transform.localPosition = new Vector3(
            viewport.transform.localPosition.x, viewport.transform.localPosition.y, 0);
        ...
        this.cellWidth = tempRect.width + spacing;
    ...
    public void ScrollToIndext(int index, bool smoothScroll = true)
    {
        if (smoothScroll)
        {
           SmoothScrollToIndex(index);
            ...
            float scrollRectWidth = scrollRect.content.offsetMax.x - scrollRect.content.offsetMin.x;
            this.scrollRect.content.offsetMin = new Vector2(-index * this.cellWidth, 0);
            this.scrollRect.content.offsetMax = new Vector2(scrollRectWidth + this.scrollRect.content.offsetMin.x, 0);
            ChangeItemPosition();
        }
    }
    ...
    private void SmoothScrollToIndex(int index) {
        float scrollRectWidth = scrollRect.content.offsetMax.x - scrollRect.content.offsetMin.x;
        float distance = Mathf.Abs(scrollRect.content.offsetMin.x + index * this.cellWidth);
        if (this.scrollRect.content.offsetMin.x > -index * this.cellWidth)
        {
            //  right
            StartCoroutine(ScrollRight(scrollRectWidth, index, distance));
        }
        else {
            // left
            StartCoroutine(ScrollLeft(scrollRectWidth, index, distance));
        }
    }


    private IEnumerator ScrollRight(float scrollRectWidth, int index, float distance) {
        while (this.scrollRect.content.offsetMin.x > - index * this.cellWidth)
        {
            yield return new WaitForEndOfFrame();
            this.scrollRect.content.offsetMin = new Vector2(Mathf.Max(this.scrollRect.content.offsetMin.x - distance / 40, -index * this.cellWidth), 0);
            this.scrollRect.content.offsetMax = new Vector2(scrollRectWidth + this.scrollRect.content.offsetMin.x, 0);
            ChangeItemPosition();
        }
    }
    ...
    private IEnumerator ScrollLeft(float scrollRectWidth, int index, float distance)
    {
        while (this.scrollRect.content.offsetMin.x < -index * this.cellWidth)
        {
            this.scrollRect.content.offsetMin = new Vector2(Mathf.Min(this.scrollRect.content.offsetMin.x + distance / 40, -index * this.cellWidth), 0);
            this.scrollRect.content.offsetMax = new Vector2(scrollRectWidth + this.scrollRect.content.offsetMin.x, 0);
            ChangeItemPosition();
            yield return new WaitForEndOfFrame();
        }
