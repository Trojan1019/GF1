...
{
    private const string OutputPath = "Assets/RawAsset/Fonts/Emoji/";
    private const string InputPath = "/Editor/Emoji/";
    ...
    private const int EmojiSize = 256; //the size of emoji.

    [MenuItem("Tools/Build Emoji")]
    public static void BuildEmoji()
    {
        ...
        Texture2D dataTex = new Texture2D((int)texSize.x / EmojiSize, (int)texSize.y / EmojiSize, TextureFormat.ARGB32,
            false);
        int x = 0;
        int y = newTex.height - EmojiSize;
        int keyindex = 0;
        ...
        using (StreamWriter sw = new StreamWriter(OutputPath + "emoji.txt", false))
        {
            ...
            {
                sw.WriteLine("{" + key + "}\t" + emojiDic[key].key + "\t" + sourceDic[key] + "\t" + emojiDic[key].x +
                             "\t" + emojiDic[key].y + "\t" + emojiDic[key].size);
            }
